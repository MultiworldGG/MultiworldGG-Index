...
**slug**
: The lowercase alphanumeric identifier for an apworld, derived from the game
name. Used as the folder name (`worlds/<slug>/`), the wheel package name
(`worlds.<slug>`), and the Index manifest filename (`worlds/<slug>.json`).
Also called `WORLD_FOLDER_NAME` in CI configuration. Example: game "Clique"
has slug `clique`.

**WORLD_FOLDER_NAME**
: The repository variable you set in Settings → Secrets and variables →
Actions → Variables. Its value is the apworld slug. The reusable build
workflow reads this variable to know which `worlds/<slug>/` subdirectory to
build. See the [easy setup guides](easy/index.md).
...
`https://github.com/you/repo/releases/download/clique-1.0.0/worlds.clique-1.0.0-py3-none-any.whl#sha256=<hex>`.

**Oliver** (Oliver-Multiworld-Squirrel)
: The GitHub App that watches per-world repos for completed
`Create and Release Python Package` workflow runs and automatically opens PRs
on the MultiworldGG-Index. Oliver pins the wheel asset URL with a SHA256 hash
so pip verifies the downloaded bytes. See [Why Oliver opens the PR](why-oliver.md).
