...
portion of your release tag (e.g. tag `clique-1.2.0` requires
`world_version: "1.2.0"`).
...
| Example | `"https://github.com/you/your-repo/releases/download/clique-1.2.0/worlds.clique-1.2.0-py3-none-any.whl#sha256=<hex>"` |
...
| Required | no |
| Written by | Author |
| Example | `["NewSoupVi", "Exempt-Medic"]` |
...
| Type | integer |
| Required | no |
| Written by | **Index operator** (never the author) |

IGDB game ID. Added by the Index's IGDB-lookup workflow after merge, not by
the author. If your game has a label "Needs IGDB id" on the Index PR, a
maintainer will add it. Do not put this in your source `archipelago.json`.
...
## Index-controlled fields

The following fields appear in the Index manifest (`worlds/<apworld>.json` on
`main`) but must never be set by the author in their source `archipelago.json`:

| Field | Who sets it | When |
|---|---|---|
| `module_location` | Oliver | When opening the Index PR |
| `igdb_id` | Index operator / IGDB-lookup workflow | After merge |

If you put these fields in your source `archipelago.json`, Oliver will
overwrite them on the next release PR. The Index manifest is the canonical
location; your source file is the input.
